...
function SessionRow({ session }: SessionRowProps) {
  ...
  const modeLabel = session.mode === 'test' ? 'Test' : 'Study';
  const modeBadgeColor = session.mode === 'test' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700';
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
      <div className="flex items-center gap-3">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full ${modeBadgeColor}`}>
          {modeLabel}
        </span>
      </div>
