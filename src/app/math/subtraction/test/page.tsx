...
import { generateSubtractionTestModeProblems } from '@/lib/game-logic/subtraction';
...
export default function SubtractionTestPage() {
  const { user } = useAuth();
  ...
  const handleStart = async (selectedNumber: number): Promise<void> => {
    ...
    const testProblems = generateSubtractionTestModeProblems(selectedNumber);
    setProblems(testProblems);

    // Create session in database
    const session = await createSession(user.id, 'subtraction', 'test');
    if (session) {
  ...
  if (testState === 'selection') {
    return <NumberSelection operation="subtraction" onStart={handleStart} />;
  }
