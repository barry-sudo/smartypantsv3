...
import { generateAdditionTestModeProblems } from '@/lib/game-logic/addition';
...
export default function AdditionTestPage() {
  const { user } = useAuth();
  ...
  const handleStart = async (selectedNumber: number): Promise<void> => {
    ...
    const testProblems = generateAdditionTestModeProblems(selectedNumber);
    setProblems(testProblems);

    // Create session in database
    const session = await createSession(user.id, 'addition', 'test');
    if (session) {
  ...
  if (testState === 'selection') {
    return <NumberSelection operation="addition" onStart={handleStart} />;
  }
